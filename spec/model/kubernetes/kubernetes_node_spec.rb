...
RSpec.describe KubernetesNode do
  ...
  describe "#observe_metrics_backlog" do
    ...
    it "resolves the page when the backlog is back within limits" do
      ...
      expect(page.reload.semaphores.map(&:name)).to eq ["resolve"]
    end
    ...
    it "keeps the page when the backlog is still above the resolve threshold" do
      ...
      expect(page.reload.semaphores).to eq []
    end

    it "does not raise when the backlog check fails" do
      expect(ssh_session).to receive(:_exec!).with(find_command).and_raise(Net::SSH::Exception.new("SSH error"))
